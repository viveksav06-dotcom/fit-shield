...
const LoginSheet: React.FC<LoginSheetProps> = ({ onNext, onBack }) => {
  const [inputValue, setInputValue] = useState('');

  const isValid = inputValue.length > 3; // Basic validation for demo

  return (
    <div className="flex flex-col w-full min-h-screen bg-[#0F0E11] relative overflow-hidden">
      {/* Background/Landing context for the sheet */}
      <div 
      ...
      <button 
        onClick={onBack}
        className="absolute top-12 left-6 p-2 rounded-full bg-[#222328] border border-[#2F2F2F] text-white z-10"
      >
      ...
      <div className="mt-auto w-full bg-[#222328] rounded-t-[24px] pt-5 pb-9 flex flex-col items-center gap-5 relative z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] border-t border-[#404040]/30 animate-in slide-in-from-bottom duration-500">
        ...
        <div className="flex flex-col items-center gap-5 w-full px-4">
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="flex flex-col items-center gap-2">
              <svg width="89" height="18" viewBox="0 0 89 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M12.9925 0C12.4376 0 11.962 0.1928 11.5835 0.575835C11.2051 0.958869 11.0107 1.43445 11.0107 1.99229C11.0107 2.55014 11.2025 3.02829 11.5835 3.40875C11.962 3.78922 12.4376 3.98201 12.9925 3.98201C13.532 3.98201 13.9999 3.78921 14.3784 3.40618C14.7568 3.02572 14.9512 2.54756 14.9512 1.98972C14.9512 1.43188 14.7594 0.953733 14.3784 0.573269C13.9974 0.192805 13.5346 0 12.9925 0Z" fill="white"/>
          ...
          {/* Heading */}
          <h2 className="text-white text-[20px] font-bold text-center w-[264px]" style={{ fontFamily: 'Quicksand' }}>
            Phone number or Email
          </h2>

          {/* Description */}
          <p className="text-[#CCC] text-[12px] font-medium text-center px-6 leading-normal" style={{ fontFamily: 'Quicksand' }}>
            Next time we'll make your recommendation ready
          </p>
        </div>

        {/* Input and Button Area */}
        <div className="w-full flex flex-col gap-5 mt-2">
          <div className="w-full h-[46px] px-4 flex items-center rounded-[12px] bg-[#2F2F39] shadow-[0_0_23px_rgba(0,0,0,0.25)] border border-transparent focus-within:border-[#00C600] transition-all">
            <input 
              type="text" 
              placeholder="Mobile number or Mail id"
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              className="w-full bg-transparent border-none outline-none text-white text-[14px] font-medium placeholder:text-[#CCC]/50 text-center"
        ...
        {/* iOS style home indicator at the very bottom */}
